...
type Query func(model.User) bool
...
type UserRepository interface {
	...
	Select(query Query) (user model.User, found bool)
	SelectMany(query Query, limit int) (results []model.User)

	InsertOrUpdate(user model.User) (updatedUser model.User, err error)
	Delete(query Query, limit int) (deleted bool)
}

// NewUserRepository returns a new user memory-based repository,
// the one and only repository type in our example.
func NewUserRepository(source map[int64]model.User) UserRepository {
...
func (r *userMemoryRepository) Select(query Query) (user model.User, found bool) {
	found = r.Exec(query, func(m model.User) bool {
		user = m
		return true
	}, 1, ReadOnlyMode)

	// set an empty model.User if not found at all.
	if !found {
		user = model.User{}
	}
...
// SelectMany same as Select but returns one or more model.User as a slice.
// If limit <=0 then it returns everything.
func (r *userMemoryRepository) SelectMany(query Query, limit int) (results []model.User) {
	r.Exec(query, func(m model.User) bool {
...
func (r *userMemoryRepository) InsertOrUpdate(user model.User) (model.User, error) {
	id := user.ID
	...
	current, exists := r.Select(func(m model.User) bool {
		return m.ID == id
	})

	if !exists { // ID is not a real one, return an error.
		return model.User{}, errors.New("failed to update a nonexistent user")
	}
...
func (r *userMemoryRepository) Delete(query Query, limit int) bool {
	return r.Exec(query, func(m model.User) bool {
